...
public class CqlSessionTracingTests extends IntegrationTestsSupport {
	...
	void shouldNotCreateAnyMetricsWhenThereIsNoObservation() {
		...
		registry.observationConfig().observationHandler(new CqlSessionTracingObservationHandler(tracer));
	...
	void tracingNoStatementsShouldProduceNoMetrics() {
		...
		CqlSessionTracingFactory.wrap(session, registry, tagsProvider);
	...
	void shouldCreateObservationForCqlSessionOperations() {
		...
		Observation.start("test", registry).scoped(() -> {
			...
			CqlSession traceSession = CqlSessionTracingFactory.wrap(session, registry, tagsProvider);
		...
		MeterRegistryAssert.then(registry).hasTimerWithNameAndTags(CASSANDRA_QUERY_OBSERVATION.getName(), Tags.of( //
